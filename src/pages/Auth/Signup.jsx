...
const Signup = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const createUserResponse = await fetch('https://Insurelink.onrender.com/auth/', {
        method: 'POST',
        headers: {
      ...
      if (!createUserResponse.ok) {
        ...
        const errorMessageText = errorData.detail?.[0]?.msg || errorData.detail || `Signup failed with status: ${createUserResponse.status}. Please try again.`;
      ...
      setSuccessMessage('Account created successfully! Sending verification code...');
      const sendOtpResponse = await fetch('https://Insurelink.onrender.com/auth/confirm-email/', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (sendOtpResponse.ok) {
        ...
        const errorMessageText = errorData.detail?.[0]?.msg || errorData.detail || `Failed to send verification code with status: ${sendOtpResponse.status}. Please try again.`;
        setErrorMessage(errorMessageText);
      ...
      <div className="flex-1 bg-white flex items-center justify-center px-6 lg:px-12 py-4">
        <div className="w-full max-w-md">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="relative">
              <select
                ...
                className={`peer w-full px-4 pt-5 pb-2 border-2 rounded-lg focus:outline-none appearance-none transition-all duration-200 cursor-pointer ${
                  aboutYourselfFocused ? 'border-[#FF7043]' : 'border-gray-200 focus:border-[#FF7043]'
                } focus-orange ${aboutYourself === '' ? 'text-gray-400' : 'text-gray-900'}`}
                required
              >
            ...
            <div className="relative">
              <select
                ...
                className={`peer w-full px-4 pt-5 pb-2 border-2 rounded-lg focus:outline-none appearance-none transition-all duration-200 cursor-pointer ${
                  insureLinkReasonFocused ? 'border-[#FF7043]' : 'border-gray-200 focus:border-[#FF7043]'
                } focus-orange ${insureLinkReason === '' ? 'text-gray-400' : 'text-gray-900'}`}
                required
              >
            ...
            <div className="flex items-start py-1">
              ...
              <label htmlFor="agreeTerms" className="ml-2 block text-sm text-gray-900 cursor-pointer">
                I agree to all the{' '}
                <a href="#" className="font-medium text-[#FF7043] hover:text-orange-600">Terms</a>
                {' '}and{' '}
                <a href="#" className="font-medium text-[#FF7043] hover:text-orange-600">Privacy Policies</a>
              </label>
            </div>
            {/* Signup Button */}
            <button
              ...
              className={`w-full py-2.5 px-4 rounded-lg font-medium cursor-pointer transition-colors ${
                isSubmitting ? 'bg-gray-400 text-gray-200 cursor-not-allowed' : 'bg-[#FF7043] text-white hover:bg-[#E55A35]'
              }`}
            >
          ...
          <p className="text-center text-sm text-gray-600 mt-4">
            ...
            <a href="/login" className="font-medium text-[#FF7043] hover:text-[#E55A35] font-medium transition-colors cursor-pointer">
              Sign in
            </a>
          </p>
