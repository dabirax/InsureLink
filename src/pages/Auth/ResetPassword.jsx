...
const ResetPassword = () => {
  ...
  // Get email from navigation state (passed from VerifyCode page after OTP verification)
  ...
  // Redirect to login if no email is found (user landed here incorrectly)
  useEffect(() => {
    if (!email) {
      navigate('/login', { replace: true }); // Redirect and replace history entry
    }
  }, [email, navigate]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Backend endpoint for resetting password: POST /auth/reset-password/
      const response = await fetch('https://Insurelink.onrender.com/auth/reset-password/', {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          ...
          // The backend might also expect the OTP here if it wasn't consumed by verify-email-otp
          // For now, we assume verify-email-otp handles the OTP state on the backend.
          // If your backend's reset-password endpoint *also* needs the OTP,
          // you'd need to pass it from the VerifyCode page's state.
        }),
      });
      ...
      if (response.ok) {
        ...
        if (contentType && contentType.includes('application/json')) {
          ...
          setErrorMessage(errorData.detail || `Password reset failed: ${response.status}. Please try again.`);
        } else {
          const errorText = await response.text();
          console.error('Reset password error: Non-JSON response from server.', errorText);
          setErrorMessage(`An unexpected server error occurred (Status: ${response.status}). Please try again later.`);
        }
      }
    } catch (error) {
      ...
      if (error instanceof TypeError && error.message.includes('Failed to fetch')) {
        ...
        setErrorMessage('Server returned an unreadable response. Please check backend logs.');
      } else {
    ...
    <div className="min-h-screen flex" style={{ fontFamily: 'Poppins, sans-serif' }}>
      ...
      <div className="flex-1 bg-white flex items-center justify-center px-6 lg:px-12 py-4">
        <div className="w-full max-w-md">
          ...
          <div className="mb-4">
            <button
              onClick={() => navigate('/login')} // Always go back to login from here
          ...
          <div className="mb-6">
            ...
            <p className="text-gray-600 text-sm">
              ...
              {email && <span className="font-medium text-[#FF7043]"> ({email})</span>}
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="relative">
              <input
                ...
                className={`peer w-full px-4 pt-5 pb-2 pr-12 border-2 rounded-lg focus:outline-none transition-all duration-200 placeholder-transparent ${
                  newPasswordFocused ? 'border-[#FF7043]' : 'border-gray-200 focus:border-[#FF7043]'
                } focus-orange`}
                placeholder="••••••••••••"
                minLength="6"
                required
            ...
            <div className="relative">
              <input
                ...
                className={`peer w-full px-4 pt-5 pb-2 pr-12 border-2 rounded-lg focus:outline-none transition-all duration-200 placeholder-transparent ${
                  confirmNewPasswordFocused ? 'border-[#FF7043]' : 'border-gray-200 focus:border-[#FF7043]'
                } focus-orange`}
                placeholder="••••••••••••"
                minLength="6"
                required
              ...
              <label
                ...
                className={`absolute left-3 sm:left-4 px-1 bg-white transition-all duration-200 pointer-events-none
                  ${confirmNewPasswordFocused || confirmNewPassword
                    ? 'top-0 -translate-y-1/2 text-xs text-gray-500'
                    : 'top-1/2 -translate-y-1/2 text-gray-400'
                  }`}
              >
          ...
          {/* Social Login Buttons (Replicated from Login/Signup) */}
      ...
      {/* Right Section - Reset Password Illustration */}
